...
void app_main()
{
    ...
    //Initialize WiFi --> will enable later, for now placeholder
    // ==== to be enabled later start ====
    // if (wifi_init_softap() != ESP_OK) {
    //     ESP_LOGE(MAIN_TAG, "Failed to initialize WiFi");
    //     abort();
    // }
    // ==== to be enabled later end =====
    xEventGroupSetBits(g_evt, B_WIFI_CONNECTED);
    
    // Wi-Fi initialized, bind first to the GUI
    // ==== to be enabled later start ====
    // ret = bind_to_gui(); 
    // if (ret != ESP_OK) {
    ...
    // Hardware initialization for ESP-BIOGAP communication. 
    pin_mux_init();
