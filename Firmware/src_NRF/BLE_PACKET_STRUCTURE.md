...
3. Streaming data packets (device → host)
   ...
   - [3.2 IMU (LIS2DUXS12 accelerometer)](#32-imu-lis2duxs12-accelerometer)
...
| `0x56` | `0x57` | 128 | IMU (accelerometer) data | device → host |
...
| 12–42 | — | 1–12 | Command code | host → device |
...
## 3.2 IMU (LIS2DUXS12 accelerometer)

3-axis accelerometer, 400 Hz, 20 samples per packet.

- **Header:** `0x56` (`IMU_DATA_HEADER`) · **Trailer:** `0x57` (`IMU_DATA_TRAILER`)
- **Total length:** 128 bytes (`IMU_PCKT_SIZE` = 1+2+4+120+1)
- **Samples per packet:** 20 (`IMU_SAMPLES_PER_PACKET`) → ~20 packets/s
...
| 1-2 | Packet counter | uint16 LE |
| 3-6 | Timestamp (µs) | uint32 LE |
| 7-126 | 20 samples × 6 bytes | see per-sample block |
| 127 | Trailer (`0x57`) | uint8 |

### Per-sample block (6 bytes)
...
Absolute offset of sample *s* (0-based) = `7 + s*6`. Default full scale is ±8 g
(`LIS2DUXS12_8g`, CTRL5 = 0xA2), so raw LSB → mg via ×0.244 (see [Scaling](#6-scaling-and-units)).
...
### Battery / status — response to command 17 (7 bytes)

| Byte | Field name | Data type | Notes |
|---|---|---|---|
| 0 | Command echo (`17`) | uint8 | `REQUEST_BATTERY_STATE` |
| 1 | Charging flag | uint8 | 1 = charging, 0 = not |
| 2 | Reserved / padding | uint8 | 0 |
| 3 | Power (mW) | uint8 | truncated to 8 bits |
| 4 | State of charge (%) | uint8 | 0–100 |
| 5 | Voltage (mV) | uint8 | **truncated** to 8 bits (legacy behaviour) |
...
| IMU accel | int16 BE | ±8 g full scale → `mg = raw × 0.244` (`lis2duxs12_from_fs8g_to_mg`) | `imu/driver/lis2duxs12_reg.c`, `lis2duxs12_sensor.c` |
